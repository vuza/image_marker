...
/**
 * $Id: DataMatrix2D.hpp 264 2015-07-17 14:17:45Z hannes $
 */

#ifndef JVIS_DATA_MATRIX_2D_HPP
#define JVIS_DATA_MATRIX_2D_HPP
...
public:
  ...
  typedef SmartPtr< ::jvis::DataMatrix2D<T> > Ptr;
  typedef SmartPtr< ::jvis::DataMatrix2D<T> const> ConstPtr;
};
...
public:
  ...
  typedef SmartPtr< ::jvis::DataMatrix2Df > Ptr;
  typedef SmartPtr< ::jvis::DataMatrix2Df const> ConstPtr;
};

/**
 * DataMatrix2Dd
 */
class DataMatrix2Dd : public DataMatrix2D<double>
{
public:
  DataMatrix2Dd() : DataMatrix2D(DOUBLE_2D){}
  DataMatrix2Dd(const int _rows, const int _cols) : DataMatrix2D<double>(_rows,_cols, DOUBLE_2D) {}
  virtual inline void push_back(const std::vector<double> &datarow);
  virtual inline void push_back(const double *d, const int _cols);
  virtual inline void setRow(const int row, const std::vector<double> &datarow);
  virtual inline void getRow(const int row, std::vector<double> &datarow) const;
  virtual inline void setTo(const double &d);
  virtual inline void setZero();

  typedef SmartPtr< ::jvis::DataMatrix2Dd > Ptr;
  typedef SmartPtr< ::jvis::DataMatrix2Dd const> ConstPtr;
};
...
public:
  ...
  typedef SmartPtr< ::jvis::DataMatrix2Db > Ptr;
  typedef SmartPtr< ::jvis::DataMatrix2Db const> ConstPtr;
};
...
public:
  ...
  typedef SmartPtr< ::jvis::DataMatrix2Di > Ptr;
  typedef SmartPtr< ::jvis::DataMatrix2Di const> ConstPtr;
};
...
template <class T>
inline T& DataMatrix2D<T>::at(const int &row, const int &col)
{
  return data[row*cols+col];
}

template <class T>
inline const T& DataMatrix2D<T>::at(const int &row, const int &col) const
{
  return data[row*cols+col];
}

template <class T>
inline T& DataMatrix2D<T>::at(const unsigned &i)
{
  return data[i];
}

template <class T>
inline const T& DataMatrix2D<T>::at(const unsigned &i) const
{
  return data[i];
}
...
/*********************************** DataMatrix2Dd *************************/
inline void DataMatrix2Dd::push_back(const std::vector<double> &datarow)
{
  data.resize(data.size()+datarow.size());
  memcpy ( &data[rows*cols], &datarow[0], sizeof(double)*datarow.size() );
  rows++;
}

inline void DataMatrix2Dd::push_back(const double *d, const int _cols)
{
  data.resize(data.size()+_cols);
  memcpy ( &data[rows*cols], d, sizeof(double)*_cols );
  rows++;
}

inline void DataMatrix2Dd::setRow(const int row, const std::vector<double> &datarow)
{
  memcpy ( &data[row*cols], &datarow[0], sizeof(double)*datarow.size() );
}

inline void DataMatrix2Dd::getRow(const int row, std::vector<double> &datarow) const
{
  datarow.resize(cols);
  memcpy ( &datarow[0], &data[row*cols], sizeof(double)*cols );
}

inline void DataMatrix2Dd::setTo(const double &d)
{
  for (int i=0; i<cols; i++)
    data[i] = d;
  for (int i=1; i<rows; i++)
    memcpy ( &data[i*cols], &data[0], sizeof(double)*cols );
}

inline void DataMatrix2Dd::setZero()
{
  memset ( &data[0], 0, sizeof(double)*data.size() );
}
