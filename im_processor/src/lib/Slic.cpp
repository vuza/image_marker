/**
 * $Id: Slic.cpp 214 2015-03-28 21:54:52Z hannes $
 *
...
{
  im_lab = cv::Mat_<cv::Vec3b>(im_rgb.size());
 ...
 * drawMeanColors
 */
void Slic::drawMeanColors(cv::Mat_<cv::Vec3b> &im_draw, const cv::Mat_<cv::Vec3b> &im_rgb, const cv::Mat_<int> &labels, int numlabels)
{
  double inv_cnt;
  std::vector<cv::Vec3d> cols(numlabels, cv::Vec3d(0.,0.,0));
  std::vector<int> cnts(numlabels, 0);

  for (int v=0; v<im_rgb.rows; v++)
  {
    for (int u=0; u<im_rgb.cols; u++)
    {
      cv::Vec3d &col = cols[labels(v,u)];
      const cv::Vec3b &rgb = im_rgb(v,u);
      col[0] += rgb[0];
      col[1] += rgb[1];
      col[2] += rgb[2];
  ...
  im_draw = cv::Mat_<cv::Vec3b>(labels.size());
  for (int v=0; v<im_rgb.rows; v++)
  {
    for (int u=0; u<im_rgb.cols; u++)
    {
      cv::Vec3d &col = cols[labels(v,u)];
      cv::Vec3b &drw = im_draw(v,u);
      drw[0] = int(col[0]);
      drw[1] = int(col[1]);
      drw[2] = int(col[2]);
    }
